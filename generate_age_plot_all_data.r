...
for(style in c(0, 1)) {
  ...
  for(county in c(sort(counties$Zupanija), 'Hrvatska')) {
    ...
    data_to_plot <- age_reshaped_sum7
    ...
    if (style == 1) {
      ...
      sty <- scale_fill_distiller(palette="Spectral", oob = scales::squish, name='Ukupno\nu 7 dana\nna 100000\nstanovnika',
  ...
  from_date <- dmy(last_date) - days(nrow(data_to_plot) - 1)
  
  from_date <- strftime(from_date, "%d.%m.%Y.")
